...
namespace Linq2Oracle
{
    class ExpressionComparer : IComparer<Expression>
    {
        ...
        {
            var xcmpr = x.Value as IComparable;
            if (xcmpr != null)
                return xcmpr.CompareTo(y.Value);

            var ycmpr = y.Value as IComparable;
            if (ycmpr != null)
                return ycmpr.CompareTo(x.Value);

            throw new DalException(DbErrorCode.E_DB_NOT_SUPPORT_OPERATOR, "LINQ語法中有存在無法最佳化的語句, 請調整程式碼");
        }
