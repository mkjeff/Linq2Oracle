    ...
    public class OracleDB
    {
        ...
        public T[] Insert<T>(params T[] list) where T : DbEntity
        {
            ...
            using (var cmd = new OracleCommand(Table<T>.InsertSql, _conn))
            {
                ...
                if (count != list.Length) // ODP.NET Managed bug?
                    throw new DalException(DbErrorCode.E_DB_INSERT_FAIL, "Batch Insert: input " + list.Length + ", success " + count);
            }
            return list;
        }
                    ...
                    return group.GroupBy(e => e.ChangedMap, (changedColumns, g) =>
                    {
                        ...
                        using (var cmd = this.CreateCommand())
                        {
                            ...
                            for (int i = 0, cnt = Table<T>.PkColumns.Length; i < cnt; i++)
                            {
                                if (i != 0) sql.Append(" AND ");
                                var c = Table<T>.PkColumns[i];
                                sql.Append(c.QuotesColumnName).Append(" = ").AppendParam(cmd.Parameters, c.DbType, g.Select(t => CheckPkNull(c.ColumnName, t.ChangedMap.TryGetValue(c.ColumnIndex, out value) ? value : c.GetValue(t))).ToArray());
                            }
                            ...
                            if (Table<T>.FixedColumns.Length != 0)
                            {
                                ...
                                for (int i = 0, cnt = Table<T>.FixedColumns.Length; i < cnt; i++)
                                {
                                    ...
                                    if ((g.First().ChangedMap.TryGetValue(c.ColumnIndex, out value) ? value : c.GetValue(g.First())) == null)
                                        ...
                                        sql.Append(c.QuotesColumnName).Append(" = ").AppendParam(cmd.Parameters, c.DbType, g.Select(t => t.ChangedMap.TryGetValue(c.ColumnIndex, out value) ? value : c.GetValue(t)).ToArray());
                                }
                            }
