...
namespace Linq2Oracle.LinqPad
{
	...
	{
		public override string Name { get { return "Linq2Oracle Static Driver"; } }
		...
		public override List<ExplorerItem> GetSchema (IConnectionInfo cxInfo, Type customType)
		{
			...
			foreach (ExplorerItem table in topLevelProps)
				table.Children = ((Type)table.Tag)
					...
					.OrderByDescending(childItem => childItem.Icon).ThenBy(item=>item.Kind)
					.ToList();
		...
		ExplorerItem GetChildItem (ILookup<Type, ExplorerItem> elementTypeLookup, PropertyInfo childProp)
		{
			...
			var attrs = (ColumnAttribute[])childProp.GetCustomAttributes(typeof(ColumnAttribute), false);
			if (attrs.Length==0)
				return null;

			// Ordinary property:
			return new ExplorerItem(childProp.Name + " (" + FormatTypeName(childProp.PropertyType, false) + ")",
				childProp.Name.Contains("ID") && !attrs.First().IsPrimarykey ? ExplorerItemKind.ReferenceLink : ExplorerItemKind.Property, 
				attrs.First().IsPrimarykey ? ExplorerIcon.Key : ExplorerIcon.Column);
		}
